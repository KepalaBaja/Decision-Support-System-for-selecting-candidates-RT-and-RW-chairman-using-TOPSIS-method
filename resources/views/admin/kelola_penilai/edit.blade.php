...
@section('content')
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('administrator.manajemen_akun/kelola_penilai') }}">Kelola
                            Penilai</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Edit Penilai</li>
                </ol>
            </nav>
        </div>
    </div>
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h5 class="m-0 font-weight-bold text-primary">Edit Penilai</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('administrator.manajemen_akun/kelola_penilai.update', $penilai->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="name">Nama Penilai</label>
                    <input type="text" name="name" id="name" class="form-control" required
                        placeholder="Masukan Nama Penilai" value="{{ $penilai->name }}">
                </div>
                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="email" name="email" id="email" class="form-control" required
                        placeholder="Masukan Email" value="{{ $penilai->email }}">
                </div>
                <div class="form-group">
